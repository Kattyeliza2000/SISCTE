...
async function subirAGoogleDrive(archivo, onProgress) {
  ...
  // Inicializar gapi si es necesario
  ...
  // Subir archivo a Google Drive con XMLHttpRequest para progreso
  return new Promise((resolve, reject) => {
    const metadata = {
      name: `${new Date().toISOString().slice(0,10)}_${usuario.area||'SIN_AREA'}_${archivo.name}`,
    ...
    xhr.onload = () => {
      if (xhr.status === 200) {
        ...
        // Hacer el archivo público para que el admin pueda descargarlo
        ...
        reject(new Error('Error subiendo a Google Drive: ' + xhr.status));
      }
    };
